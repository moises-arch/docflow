...
export function DeleteProfileDialog({
  ...
  const t = useTranslations("templates.profileStudio.deleteProfile");
  ...
  const nameMatches = confirmText.trim() === profileName;
  const canDelete = acknowledged && nameMatches && !deleting && !isSystem;
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 422 && body.error?.includes("System")) {
          toast.error(t("systemToast"));
        } else {
      ...
      toast.success(t("successToast", { name: profileName }));
      setOpen(false);
      reset();
      router.push("/templates/profile-studio");
    } catch {
      toast.error(t("errorToast"));
      setDeleting(false);
    }
        ...
        <div className="grid gap-3 py-2">
          ...
          <div className="grid gap-1.5">
            ...
            <input
              ...
              placeholder={t("typeToConfirmPlaceholder", { name: profileName })}
              className={cn(
