...
const groups: NavGroup[] = [
  {
    heading: "MONITOREO",
    items: [
      { href: "/ingest/rithum", label: "Resumen", icon: Package2 },
      { href: "/ingest/rithum/salud", label: "Salud del sistema", icon: Activity },
    ],
  },
  {
    heading: "ÓRDENES",
    items: [
      { href: "/ingest/rithum/historial", label: "Historial", icon: ClipboardList },
    ],
  },
  {
    heading: "REFERENCIA",
    items: [
      { href: "/ingest/rithum/documentacion", label: "Documentación", icon: BookOpen },
    ],
  },
];
    ...
    <aside className="flex h-full flex-col border-r border-[var(--color-border)] bg-[var(--color-bg)]">
      ...
      <div className="border-b border-[var(--color-border)] px-4 py-4">
        <div className="inline-flex items-center gap-2.5 text-sm font-medium text-[var(--color-fg)]">
          <Package2 size={16} className="text-[var(--color-fg-mute)]" aria-hidden />
          <span>Supplier Portal / Supplier Portal</span>
        </div>
      </div>
          ...
          <div key={group.heading} className="mb-4">
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => {
                const active =
                  item.href === "/ingest/rithum"
                    ? pathname === "/ingest/rithum"
