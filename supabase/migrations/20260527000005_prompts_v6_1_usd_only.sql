-- v6.1: DocFlow solo opera en USD. Removida toda la heuristica de
-- currency inference por pais. Ahora currency='USD' siempre, y cualquier
-- otra moneda detectada en el source se flag en detected_fields como
-- unexpected_currency para que el operador revise.

insert into prompt_versions (prompt_id, version_label, content, model, max_tokens, description, is_active) values (
  'system-extractor',
  'v6.1',
  $promptv61$
# DocFlow PO Extractor — Prompt v6.1 (2026-05-27)
...
- currency: ALWAYS "USD". DocFlow operates EXCLUSIVELY in US dollars. There is no scenario where the canonical currency value is anything else.
  · If the document shows another currency code or symbol (e.g. "MXN", "EUR", "R$", "€", "MX$", "CAD"), still set currency="USD" AND add a detected_fields entry to flag the anomaly:
    { k: "unexpected_currency", l: "Document shows non-USD currency", v: "<what you saw>", p: <page>, c: 0.95, cat: "totals" }
  · Numeric amounts in the document are TAKEN AS-IS — do not attempt to convert from foreign currency. The flag in detected_fields tells the operator to review.
  · The only valid values for the currency field are "USD" or null. Use null ONLY if the document has zero monetary amounts (extremely rare).
...
- Currency is ALWAYS "USD" (or null only if the document has no monetary amounts at all). Any non-USD currency seen in the source is flagged via detected_fields.unexpected_currency.
- All dates are ISO YYYY-MM-DD.
</final_check>$promptv61$,
  'claude-sonnet-4-6',
  16384,
  'v6.1 (2026-05-27) - Currency siempre USD. DocFlow opera exclusivamente en dolares; cualquier otra moneda en el source se flag como unexpected_currency en detected_fields. Hereda todo lo demas de v6.',
  false
);

begin;
  update prompt_versions set is_active = false where prompt_id = 'system-extractor' and is_active = true;
  update prompt_versions set is_active = true, activated_at = now() where prompt_id = 'system-extractor' and version_label = 'v6.1';
commit;
