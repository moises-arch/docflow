-- Fix: la migration 20260527000003 escapó mal los newlines del prompt v6,
-- quedaron como \n literal en vez de saltos de línea reales.
-- Re-cargamos el contenido usando dollar-quoted string (sin escaping).

update prompt_versions set content = $promptv6$
# DocFlow PO Extractor — Prompt v6 (2026-05-27)
...
- currency: ISO-3 ("USD","MXN","CAD","EUR","BRL"). Heuristics:
  · Explicit code present ("USD","MXN", etc.) → use it verbatim.
  · Only "$" appears → look at billing_country / delivery_country for context:
      US or unknown → "USD". MX → "MXN". CA → "CAD". BR → "BRL".
  · "R$" → "BRL". "€" → "EUR". "MXN$" or "MX$" → "MXN".
...
## Example 3 — PO con descuento por volumen y totals mismatch
Input excerpt:
...
Output (full keys present, currency MXN inferred from MX context):
{
  "po_number":"1635065595","po_date":"2026-05-27","delivery_date":null,"currency":"MXN","payment_terms":null,
...
</final_check>$promptv6$
where prompt_id = 'system-extractor' and version_label = 'v6.0';
